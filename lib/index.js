 ...
 * @param {Logger} logger
 */
const combinePRs = async (github, target, options, logger) => {
  const {
    baseBranch,
    combineBranchName,
    mustBeGreen,
  ...
  for await (const pr of combinablePRs) {
    ...
    try {
      try {
        ...
        await applyPRVersionBump(github, pr, logger);
      }
      ...
      logger.error(`Failed to apply "${pr.title}" due to error:\n\n${err}`);
    }
  }
 ...
 */
const setupRepository = async (baseBranch, combineBranchName) => {
 ...
 * @param {Logger} logger
 */
const cherryPickPR = async (pr, logger) => {
  logger.info(
    `Cherry-picking ${pr.pkg} from ${pr.fromVersion} to ${pr.toVersion}.`
  );
  try {
    const results = await execa("git", ["cherry-pick", pr.lastCommit.sha], {
      stderr: "ignore",
    });
    return results;
  } catch (err) {
